...
open class XAxisRendererBlockChart: XAxisRenderer
{
    @objc public override init(viewPortHandler: ViewPortHandler, xAxis: XAxis?, transformer: Transformer?)
    {
    ...
    @objc open override func computeSize()
    {
        guard let
            xAxis = self.axis as? XAxisBlockChart
            else { return }
    ...
    {
        guard let xAxis = self.axis as? XAxisBlockChart else { return }
    ...
    {
        guard
            let xAxis = self.axis as? XAxisBlockChart,
        ...
        let optionalLabelAttrs: [NSAttributedString.Key : Any] = [
            .font: xAxis.optionalLabelFont,
            .foregroundColor: xAxis.optionalLabelTextColor,
            .paragraphStyle: paraStyle
        ]
        ...
        for i in stride(from: 0, to: entries.count, by: 1)
        {
            ...
            if viewPortHandler.isInBoundsX(position.x)
            {
                ...
                let optionalLabel = xAxis.optionalValueFormatter?.stringForValue(entries[i], axis: xAxis) ?? ""
                ...
                drawLabel(context: context,
                          formattedLabel: optionalLabel,
                          x: position.x,
                          y: pos - 5,
                          attributes: optionalLabelAttrs,
                          constrainedToSize: labelMaxSize,
                          anchor: anchor,
                          angleRadians: labelRotationAngleRadians)
            }
    ...
    @objc open override func drawLabel(
        context: CGContext,
    ...
    @objc open override var gridClippingRect: CGRect
    {
    ...
    @objc open override func drawGridLine(context: CGContext, x: CGFloat, y: CGFloat)
    {
    ...
    @objc open override func renderLimitLineLine(context: CGContext, limitLine: ChartLimitLine, position: CGPoint)
    {
